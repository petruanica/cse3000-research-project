...
x = df[
    ["age", "weight", "pf_ratio", "po2", "pco2", "driving_pressure", "bilirubin", "urea", "fio2", "plateau_pressure"]]
# x = df.drop(columns=["mort_28", "peep_regime", "map", "bilirubin", "creatinine", "platelets", "urea", "diastolic_blood_pressure"])
y = df["mort_28"]
t = df["peep_regime"]
...
for epochs, layers_r, layers_out, neurons_r, neurons_out, lr, batch_size, alpha in configurations:
    ...
    areas_max = np.max(area_list)
    areas[f"{config}"] = {"mean": areas_mean, "std": areas_std, "max": areas_max}
    print(f"Area --- mean: {areas_mean}, std: {areas_std}, max: {areas_max} ({len(area_list)} iterations)\n")

areas_sorted = sorted(areas.items(), key=lambda item: item[1]["mean"], reverse=True)
os.makedirs("gain", exist_ok=True)
f = open(f"gain/areas_s_learner_i{iterations}_{now}.txt", "w")
f.write("Sorted areas under the curve (mean, std, max) - S-learner:\n")
for conf, area in areas_sorted:
    f.write(f"{conf} - mean: {area['mean']}, std: {area['std']}, max: {area['max']}\n")
f.close()
