...
for epochs, layers_r, layers_out, neurons_r, neurons_out, lr, batch_size, alpha in configurations:
    config = f"e{epochs}_lyr{layers_r}_lyo{layers_out}_nr{neurons_r}_no{neurons_out}_lr{lr}_bs{batch_size}_alpha{0.0}"
    area_list = []
...
f = open(f"gain/areas_tarnet_i{iterations}_{now}.txt", "w")
f.write("Sorted areas under the curve (mean, std, max) - TARNet:\n")
